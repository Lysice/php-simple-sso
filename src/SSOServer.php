...
use Lysice\SimpleSSO\Interfaces\SSOServerInterface;
...
abstract class SSOServer implements SSOServerInterface
{
    ...
    public function attach(?string $broker, ?string $token, ?string $checksum)
    {
        try {
            if (!$broker) {
                $this->fail('No broker id specified.', true, Constants::CODE_NO_BROKER_ID_SPECIFIED);
            }

            if (!$token) {
                $this->fail('No token specified.', true, Constants::CODE_NO_TOKEN_SPECIFIED);
            }

            if (!$checksum || $checksum != $this->generateAttachChecksum($broker, $token)) {
                $this->fail('Invalid checksum.', true, Constants::CODE_CHECKSUM_FAILED);
            }
    ...
    public function login(?string $username, ?string $password)
    {
        try {
            ...
            if (!$username || !$password) {
                $this->fail('No username and/or password provided.', false, Constants::CODE_NO_USERNAME_OR_PASSWORD_PROVIDED);
            }
    ...
    public function userInfo()
    {
        try {
            ...
            if (!$username) {
                $this->fail('User not authenticated. Session ID: ' . $this->getSessionData('id'), false,
                    Constants::CODE_USER_NOT_LOGIN);
            }
    ...
    protected function startBrokerSession()
    {
        ...
        if (!$sessionId) {
            $this->fail('Missing session key from broker.', false, Constants::CODE_SESSION_KEY_MISSED);
        }

        $savedSessionId = $this->getBrokerSessionData($sessionId);

        if (!$savedSessionId) {
            $this->fail('There is no saved session data associated with the broker session id.', false,
                Constants::CODE_SESSION_DATA_MISSED);
        }
    ...
    protected function validateBrokerSessionId(string $sessionId)
    {
        ...
        if (!preg_match('/^SSO-(\w*+)-(\w*+)-([a-z0-9]*+)$/', $this->getBrokerSessionId(), $matches)) {
            $this->fail('Invalid session id', false, Constants::CODE_SESSION_ID_INVALID);
        }

        if ($this->generateSessionId($matches[1], $matches[2]) != $sessionId) {
            $this->fail('Checksum failed: Client IP address may have changed', false, Constants::CODE_IP_CHANGED);
        }
    ...
    protected function generateSessionId(string $brokerId, string $token)
    {
        ...
        if (!$broker) {
            $this->fail('Provided broker does not exist.', false, Constants::CODE_BROKER_NOT_EXIST);
        }
     ...
     * @throws SSOServerException
     */
    protected function fail(?string $message, bool $isRedirect = false, int $code = 500, ?string $url = null)
    {
        if (!$isRedirect) {
            throw new SSOServerException($message, $code);
        }
